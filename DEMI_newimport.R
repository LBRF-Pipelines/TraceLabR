...
PP_anal = function(tlf, tlt, tlfp, tlfs){
        #create data frames
...
CC_anal = function(tlf, tlfp, tlfs){
        #loads stimulus data
        data_stim <- data.frame(matrix(as.numeric(unlist(strsplit(gsub("\\[|\\]|\\(|\\)", "", as.character(tlf)), ", "))),ncol=3,nrow=length(tlf)/3, byrow=TRUE))
        ctrl_pts <- data.frame(matrix(as.numeric(unlist(strsplit(gsub("\\[|\\]|\\(|\\)", "", as.character(tlfs)), ", "))),ncol=2,nrow=length(tlfs)/2, byrow=TRUE))
        vertices <- data.frame(matrix(as.numeric(unlist(strsplit(gsub("\\[|\\]|\\(|\\)", "", as.character(tlfp)), ", "))),ncol=2,nrow=length(tlfp)/2, byrow=TRUE))
        ...
        datarow <- c(name.tlf,PLstim,sinuosity,totabscurv,turnangle_sum,turnangle_mean,turnangle_SD,ApEn,SampEn,rep(NA,times=24), corr.resp)
        return(datarow)
}

#### GATHER INFORMATION FROM DATABASE AND LOCATE DATA ####
path <- "~/TraceLab_v2/TraceLab/ExpAssets/Data/"
trials <- read.table(paste(path,'TraceLab_all_trials.txt',sep=''),skip=7,sep='\t',header=TRUE,stringsAsFactors = FALSE)
# trials <- trials[trials$participant!=69,]
# trials <- trials[trials$participant!=70,]
file.names <- dir(path, recursive = TRUE, full.names = TRUE,pattern="\\.zip$") 

#### LOAD EXISTING DATA AND IDENTIFY NEW DATA TO BE ANALYZED ####

if(file.exists("DEMI_all_data.Rda")){
        load("DEMI_all_data.Rda")
...
if(nrow(trials)==0){
        ...
        for(i in 1:length(file.names)) {
                ...
                # determine group, and run appropriate analysis CHANGE TO CONDITION (IMAGERY, CONTROL)
                if(trials[trials$figure_file==name.tlf,8]=='imagery'){
                        # finds sessions 1 to 4:
                        if(trials[trials$figure_file==name.tlf,7]!=5){
                                datarow <- MI_anal(tlf, tlfp, tlfs)
                        }
                        # otherwise it's session 5:
                        else{
                                datarow <- PP_anal(tlf,tlt,tlfp,tlfs)
                        } 
                } else if(trials[trials$figure_file==name.tlf,8]=='control'){
                        # finds sessions 1 to 4:
                        if(trials[trials$figure_file==name.tlf,4]!=5){
                                datarow <- CC_anal(tlf, tlfp, tlfs)
                        }
                        # otherwise it's session 5:
                        else{
                                datarow <- PP_anal(tlf,tlt,tlfp,tlfs)
        ...
        all_data_new$turnangle_sum <- as.numeric(all_data_new$turnangle_sum)
        all_data_new$turnangle_mean <- as.numeric(all_data_new$turnangle_mean)
        all_data_new$turnangle_SD <- as.numeric(all_data_new$turnangle_SD)
        ...
        colnames(all_data_new)[56] <- "figure_type"
        colnames(all_data_new)[11] <- "figure_name"
        ...
        # save(all_data, file = paste(path,"DEMI_all_data.Rda",sep=""))
        save(all_data, file = "DEMI_all_data.Rda")
}
